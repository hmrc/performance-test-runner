...
Csv user feeder described above should be sufficient for most data driven tests. When csv feeder is not sufficient, custom feeders can be 
created. 
...
```
setup("home", "Go to the homepage").withActions(uuidFeeder.actionBuilders:_*).withRequests(navigateToHome)
```
