...
trait Configuration {
  ...
  private[conf] val defaultConfig =
    ConfigFactory.systemProperties().withFallback(ConfigFactory.load("journeys")).withFallback(ConfigFactory.load())
  ...
  def readProperty(property: String, default: String): String =
    Try(applicationConfig.getString(property)).getOrElse(default)

  def readPropertyOption(property: String): Option[String] = Try(applicationConfig.getString(property)).toOption

  def readPropertyBooleanOption(property: String): Option[Boolean] = Try(
    applicationConfig.getBoolean(property)
  ).toOption
  ...
  def readPropertySetOrEmpty(property: String): Set[String] =
    Try(applicationConfig.getStringList(property).asScala.toSet).getOrElse(Set.empty)
